...
fn main() {
    ...
    let config: Option<Config> = if config_path.exists() && config_path.is_file() {
        ...
        fs::File::open(&config_path)
            .unwrap_or_else(|_| panic!("Unable to open file for reading: {}", config_path.to_string_lossy()))
            .read_to_string(&mut config_string)
            .unwrap_or_else(|_| panic!("Unable to read file contents: {}", config_path.to_string_lossy()));
        Some(serde_json::from_str(&config_string)
            .unwrap_or_else(|_| panic!("Unable to parse file as JSON: {}", config_path.to_string_lossy())))
    } else {
        None
    };
